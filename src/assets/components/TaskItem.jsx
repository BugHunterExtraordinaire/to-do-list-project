...
export default function TaskItem( { task, time, date, index, changePosition, tasks, setTasks } ) {
  const itemIndex = index;
  return (
   <div className="task-item">
      <button className={index === 0 ? "up-button disabled" : "up-button"} 
              onClick={() => changePosition(index, -1)}>⬆</button>
      <button className={index === tasks.length - 1 ? "down-button disabled" : "down-button"} 
              onClick={() => changePosition(index, 1)}>⬇</button>
